...
contract CortexArtERC721 is ERC721URIStorage {
    ...
    event PermissionUpdated(uint256 tokenId, address tokenOwner, address permissioned);

    // An event whenever a creator is whitelisted with the token id and the layer count
    event CreatorWhitelisted(uint256 tokenId, uint256 layerCount, address creator);
    ...
    function mintArtwork(string calldata _artworkTokenURI) external {
        ...
        _checkExpires();
        require(
            mintHistory[msg.sender].length < maximumPiecePerCreator,
            "creator exceeded maximum amount"
        );
    ...
    function mintArtwork(string[] calldata _artworkTokenURI) external {
        ...
        _checkExpires();
        require(
            mintHistory[msg.sender].length + _artworkTokenURI.length <= maximumPiecePerCreator,
            "creator exceeded maximum amount"
        );
        for (uint256 i; i < _artworkTokenURI.length; ++i) {
    ...
    function bid(uint256 _tokenId) external payable {
        ...
        if (pendingBids[_tokenId].exists) {
            ...
            require(
                msg.value >=
                    (pendingBids[_tokenId].amount.mul(minBidIncreasePercent.add(100)).div(100)),
    ...
    function onTokenSold(
        ...
        if (tokenDidHaveFirstSale[_tokenId]) {
            ...
            uint256 platformAmount = saleAmount.mul(platformSecondSalePercentages[_tokenId]).div(
                100
            );
            ...
            uint256 platformAmount = saleAmount.mul(platformFirstSalePercentages[_tokenId]).div(
                100
            );
            safeFundsTransfer(platformAddress, platformAmount);
